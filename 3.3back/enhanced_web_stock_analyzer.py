...
class EnhancedWebStockAnalyzer:
    ...
    def get_llm_trade_decision(self, stock_code, date_str, price_info, tech_indicators, money_flow):
        ...
        # ====================================================
            prompt = f"""
            你是一位拥有20年经验的**全权委托基金经理**，擅长结合**基本面舆情**与**技术面量化**进行波段交易。
            你需要摆脱僵化的指标束缚，利用你的金融直觉和逻辑推理能力，对当前标的进行独立判断。

            [核心情报档案]
            - 标的: {stock_code} (日期: {date_str})
            - 📰 **舆情摘要**: {news_summary}  <-- (🔥 核心变量：请重点评估此消息对股价的短期冲击，是利好兑现、情绪反转还是无实质影响？)
            ...
            [量化指标参考]
            - RSI(6): {rsi:.1f} (注意：>85为极度超买区，<20为极度超卖区)
            ...
            [你的决策任务 - 深度思考]
            1. **舆情否决权**: 
               - 如果舆情摘要包含重大黑天鹅（如造假、制裁、暴雷），无论技术指标多完美，必须 **SELL** 或 **HOLD**，严禁买入。
               - 如果是重大利好，请判断是否已经“利好兑现”导致高开低走？
            
            2. **真假突破辨析 (Smart Money)**: 
               - 如果相对位置 > 90%：这是强劲的主升浪（配合资金流入+利好），还是缺乏基本面支撑的诱多赶顶？
               - **RSI 陷阱**: 如果 RSI > 85 且没有实质性重大利好支撑，通常是见顶信号，不要盲目追高。

            3. **操作哲学**: 
               - 震荡期(MA20走平)首选 **左侧博弈** (低吸高抛)。
               - 趋势期(MA20向上)首选 **右侧跟随**，但要避开乖离率过大的位置。

            [最终裁决]
            请综合上述信息，给出一个最负责任的操作指令。
            - **BUY**: 仅当胜率 > 70% 且 盈亏比划算时（例如：底部启动、回踩支撑、或强势突破且未超买）。
            - **HOLD**: 趋势尚可但位置尴尬，或局势不明朗，或已有利润但未触发止损。
            - **SELL**: 趋势走坏，或处于情绪狂热的顶部（超买+滞涨），或基本面恶化。

            [输出要求]
            请仅返回一个标准的 JSON 对象（严禁Markdown格式）：
            {{
                "market_phase": "你的判断(如：情绪过热/底部反转/主升浪/阴跌中)",
                "action": "BUY", "HOLD", 或 "SELL",
                "confidence": 0到100的整数,
                "reason": "请用一句话犀利地点评核心逻辑（结合新闻与指标，如：虽有财报利好，但RSI高达90且放量滞涨，警惕利好兑现，建议观望）",
