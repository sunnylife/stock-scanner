...
def run_backtest(stock_list, backtest_days=20):
    ...
    if len(buy_signals) > 0:
        win_count = len(buy_signals[buy_signals['actual_return'] > 0])
        win_rate = (win_count / len(buy_signals)) * 100
        avg_return = buy_signals['actual_return'].mean()
        
        print(f"🟢 触发买入信号: {len(buy_signals)} 次")
        print(f"🏆 次日上涨胜率: {win_rate:.1f}%")
        print(f"💰 次日平均收益: {avg_return:.2f}%")
        
        # 打印最近几次买入信号
        print("\n📋 最近5次买入信号详情:")
        cols = ['date', 'stock', 'score', 'actual_return']
        print(buy_signals[cols].tail(5).to_string(index=False))
    else:
    ...
    if failed_cases:
        ...
        for i, case in enumerate(failed_cases[:5]):
            print(f"[{i+1}] {case['date']} {case['stock']} | 收益: {case['actual_return']:.2f}% | 原因: {case['reason']}")
    else:
        print("\n🎉 完美！没有发现严重亏损案例。")
