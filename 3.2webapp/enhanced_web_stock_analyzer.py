...
class EnhancedWebStockAnalyzer:
    ...
    def __init__(self, config_file='config_back.json'):
        ...
        # # === ✅ 新增：初始化 LLM 客户端 ===
        # self.client = None
        # self.model_name = "gpt-4o" # 默认模型
        # self._init_llm_client()   # 调用初始化辅助函数
        # # ================================
    ...
    def get_llm_trade_decision(self, stock_code, date_str, price_info, tech_indicators, money_flow):
        ...
        try:
            ...
            # ====================================================
            prompt = f"""
            你是一名精通“趋势跟踪”与“波段博弈”的量化基金经理。请基于以下数据先识别市场阶段，再给出操作指令。

            [核心数据]
            - 标的: {stock_code} (日期: {date_str})
            - 价格: {close} (涨跌: {change_pct:.2f}%)
            - 趋势状态: 20日线斜率 {ma20_slope:.4f} ({'向上' if ma20_slope > 0.001 else '走平' if ma20_slope > -0.001 else '向下'})
            - 相对位置: {pp_20d:.1f}% (0=箱底, 100=箱顶/突破)
            - 资金强度: {money_flow_score:.1f}/100 (量比: {vol_ratio:.2f})
            - 波动风险(ATR): {atr:.3f} (占比{atr_percent:.1f}%)

            [技术指标 - 增强版]
            - RSI(6): {rsi:.1f} (强弱分界)
            - KDJ(9,3,3): K={k_value:.1f}, D={d_value:.1f} (信号: {kdj_signal})
            - 威廉WR(14): {wr:.1f} ({'超卖区' if wr > 80 else '超买区' if wr < 20 else '中性'})
            - ★信号共振★: {resonance_str}

            [决策逻辑核心：先定性，再定策]
            请根据以下特征判定当前处于哪个阶段，并执行对应策略：

            1. **🚀 突破期 (Breakout)**
               - *特征*: 相对位置 > 90% + 资金强度 > 60 + 趋势向上/走平。
               - *策略*: **BUY (追涨)**。这是主升浪起点，允许RSI略高，重点看资金配合。

            2. **🔥 爬升期 (Up-Trend)**
               - *特征*: 20日线斜率明显向上，且相对位置在 20%-80% 之间。
               - *策略*: **HOLD (持股)**。若出现“急跌回踩MA20”或“RSI<40”，视为**BUY (低吸)**机会。

            3. **📦 震荡期 (Consolidation)**
               - *特征*: 20日线走平，价格在箱体内部波动。
               - *策略*: **左侧博弈**。若出现“底部共振(RSI低位+KDJ金叉)”或“WR超卖”，可尝试 **BUY (博弈)**；否则观望。

            4. **☠️ 下降期 (Down-Trend)**
               - *特征*: 20日线斜率向下，或相对位置 < 20% 且无资金承接。
               - *策略*: **SELL/HOLD (防守)**。严禁抄底，除非出现极度超卖的三指标共振。

            [风控一票否决]
            - 若 ATR 占比 > 5% (剧烈波动) 且方向不明，强制观望。
            - 若 WR 进入超买区(<20) 且资金分数 < 50，警惕诱多，禁止追高。

            [输出要求]
            请仅返回一个标准的 JSON 对象（不要Markdown格式，不要 ```json 开头）：
            {{
                "market_phase": "突破期 / 爬升期 / 震荡期 / 下降期",
                "action": "BUY", "HOLD", 或 "SELL",
                "confidence": 0到100的整数,
                "reason": "简短说明(如: 处于震荡期底部，且出现RSI金叉共振)",
                "risk_warning": "当前最大风险点"
            }}
            """
